    ...
    while (true)
    {
        ...
        if (!armors.empty())
        {
            ...
            std::vector<cv::Point2f> img_points{
                armor.left.top, armor.right.top, armor.left.bottom, armor.right.bottom};
            //
            // 提示：
            // - 看看 Armor 结构体有哪些成员。
            ...
            tools::draw_text(img, fmt::format("rvec:  x{: .2f} y{: .2f} z{: .2f}", rvec.at<double>(0), rvec.at<double>(0), rvec.at<double>(0)), cv::Point2f(10, 120), 1.7, cv::Scalar(0, 255, 255), 3);
            //
            // 提示：
